...
import { Package, Clock, CheckCircle, Bike, ShoppingBag, XCircle, ChefHat, MessageCircle, MapPin, Plus, Trash2, Home, Briefcase, PlusCircle, ArrowLeft } from 'lucide-react';
...
const CustomerDashboard: React.FC<CustomerDashboardProps> = ({ orders, restaurants, user, onBackToHome }) => {
  ...
  const handleSaveAddress = async (e: React.FormEvent) => {
    ...
    // Forçar atualização do componente (em um app real usaríamos um context ou state lift)
  ...
  const getStatusInfo = (status: OrderStatus, orderType: string) => {
     switch(status) {
       case 'pending': return { label: 'Aguardando', icon: <Clock size={14}/>, color: 'text-yellow-600', bg: 'bg-yellow-100' };
       case 'preparing': return { label: 'Preparando', icon: <ChefHat size={14}/>, color: 'text-blue-600', bg: 'bg-blue-100' };
       ...
       case 'heading_to_pickup': return { label: 'Entregador indo Buscar', icon: <MapPin size={14}/>, color: 'text-orange-600', bg: 'bg-orange-100' };
                   ...
                   <div key={order.id} className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                     <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                           ...
                           <div>
                              ...
                              <p className="text-[10px] text-gray-400 uppercase font-bold">{new Date(order.timestamp).toLocaleDateString()}</p>
                           </div>
                        </div>
                     ...
                     <div className="p-4">
                        ...
                        <div className="flex justify-between items-center pt-3 border-t border-dashed">
                           <span className="text-xs font-bold text-gray-400">{order.paymentMethod}</span>
                           <span className="font-bold text-brand-600">Total R$ {order.total.toFixed(2)}</span>
                        </div>
                     </div>
                   </div>
                 );
                  ...
                  {myAddresses.map(addr => (
                    <div key={addr.id} className="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm flex items-center gap-4 group">
                       ...
                       <div className="flex-1">
                          ...
                          <p className="text-[10px] text-gray-400 font-medium uppercase">{addr.neighborhood}, Arari</p>
                       </div>
                       <button 
              ...
              <form onSubmit={handleSaveAddress} className="bg-white p-6 rounded-3xl border shadow-md space-y-4 animate-slideUp">
                 ...
                 <div className="grid grid-cols-4 gap-3">
                    <div className="col-span-3">
                      ...
                      <input required value={addressForm.street} onChange={e => setAddressForm({...addressForm, street: e.target.value})} className="w-full border rounded-xl p-3 text-sm bg-gray-50 outline-none focus:ring-2 focus:ring-brand-500" placeholder="Ex: Avic. Dr. Paulo Ramos" />
                    </div>
                    <div>
