...
const RestaurantDashboard: React.FC<RestaurantDashboardProps> = ({ 
  ...
  // Logic to detect courier pickup and play sound
  const prevOrdersRef = useRef<Order[]>(myOrders);

  useEffect(() => {
    // Check if any order changed to 'heading_to_pickup'
    const newPickup = myOrders.find(o => {
        const prev = prevOrdersRef.current.find(p => p.id === o.id);
        // Trigger if it wasn't heading_to_pickup before, and now it is
        return prev && prev.status !== 'heading_to_pickup' && o.status === 'heading_to_pickup';
    });
    ...
    if (newPickup) {
        ...
        if ("Notification" in window && Notification.permission === "granted") {
            new Notification("Entregador a Caminho!", { body: `Pedido #${newPickup.id.slice(-4)} aceito por ${newPickup.courierName}` });
        }
    }
    
    prevOrdersRef.current = myOrders;
  }, [orders]); // Depend on global orders update

  // Logic to check if open (multi-shift)
  const now = new Date();
  ...
  const formatHours = () => {
      let text = `${restaurant.openingTime} - ${restaurant.closingTime}`;
      if (restaurant.openingTime2 && restaurant.closingTime2) {
          text += ` / ${restaurant.openingTime2} - ${restaurant.closingTime2}`;
      }
      return text;
  };
          ...
          {myOrders.map(order => (
            <div key={order.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col hover:shadow-md transition-shadow">
              ...
              <div className="p-4 flex-1 bg-gray-50/50">
                 ...
                 {/* Mostrar quem é o entregador se estiver em rota ou indo buscar */}
                 {(order.status === 'delivering' || order.status === 'heading_to_pickup') && order.courierName && (
                    <div className={`mt-2 text-xs px-2 py-1 rounded border flex items-center gap-1 animate-pulse ${order.status === 'heading_to_pickup' ? 'bg-orange-50 text-orange-700 border-orange-200' : 'bg-purple-50 text-purple-700 border-purple-100'}`}>
                        <Bike size={14} />
              ...
              <div className="p-3 border-t border-gray-100 bg-white">
                 ...
                 <div className="grid grid-cols-1 gap-2">
                   ...
                   {order.status === 'heading_to_pickup' && (
                     <div className="w-full py-2 bg-orange-100 text-orange-700 rounded-lg font-bold text-sm text-center border border-orange-200 flex items-center justify-center gap-2 shadow-sm animate-pulse">
